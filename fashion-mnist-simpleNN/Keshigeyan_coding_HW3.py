...
def get_data(batch_size_train, batch_size_valid, batch_size_test):
    ...
    # Create dataloaders for train and validation. (Note that train set == validation set in this question. We will do correctly in the following weeks)
    train_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size_train, sampler=train_sampler)
    valid_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size_valid, sampler=train_sampler)
